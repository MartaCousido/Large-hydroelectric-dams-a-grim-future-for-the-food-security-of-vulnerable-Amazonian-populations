...
Xgamma <- data.frame(Intercgamma = rep(1, N), 
                    INFLUENCIAgamma   = (data1$INFLUENCIA),
...
Xbeta <- data.frame(Intercbeta = rep(1, N), 
                    INFLUENCIAbeta   = (data1$INFLUENCIA),
...
#### Model Time linear (without vazao) {.tabset}
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE}
measures=rbind(vec1,vec2,vec3,vec4)
colnames(measures)=c("waic","dic")
rownames(measures)=c("Full model","Without Season", "Without vazao", "Time linear (without vazao)")
measures
```


Best model the FULL one.
...
#### River level rate {.tabset}


```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE}

library(ggplot2)
suabinm <- I4$summary.random$grouped_vazaobeta$mean
suabin2 <- I4$summary.random$grouped_vazaobeta$`0.025quant`
suabin9 <-I4$summary.random$grouped_vazaobeta$`0.975quant`
suabinID<-I4$summary.random$grouped_vazaobeta$ID
suabin<-data.frame(suabinm, suabin2,suabin9,suabinID)
m3<-ggplot(data = suabin, aes(x = suabinID, y = suabinm))+
  geom_line(aes(x = suabinID, y = suabinm), color="chocolate", size=0.9)+
  geom_ribbon(aes(ymin=suabin2, ymax=suabin9) ,fill="chocolate", alpha=0.15)+
  ggtitle(" ")+
  xlab("River level rate ")+
  ylab("River level rate effect (beta) ")+
  theme_light() + theme(plot.title = element_text(hjust=0.5))+theme_classic()
m3
```
...
library(ggplot2)
suabinm <- I4$summary.random$grouped_vazaogamma$mean
suabin2 <- I4$summary.random$grouped_vazaogamma$`0.025quant`
suabin9 <-I4$summary.random$grouped_vazaogamma$`0.975quant`
suabinID<-I4$summary.random$grouped_vazaogamma$ID
suabin<-data.frame(suabinm, suabin2,suabin9,suabinID)
m3<-ggplot(data = suabin, aes(x = suabinID, y = suabinm))+
  geom_line(aes(x = suabinID, y = suabinm), color="chocolate1", size=0.9)+
  geom_ribbon(aes(ymin=suabin2, ymax=suabin9) ,fill="chocolate1", alpha=0.15)+
  ggtitle(" ")+
  xlab("River level rate ")+
  ylab("River level rate effect (gamma) ")+
  theme_light() + theme(plot.title = element_text(hjust=0.5))+theme_classic()
m3
```
